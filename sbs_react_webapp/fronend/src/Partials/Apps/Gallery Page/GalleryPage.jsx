...
const GalleryPage = () => {
    ...
    const fetchImages = async () => {
        try {
            const response = await axios.get(`${APP_URL}/api/gallery/${shopId}`, config);
    ...
    useEffect(() => {
        ...
        if (shopId || refreshTrigger) {
            fetchImages();
        }
    }, [shopId, token, refreshTrigger]);
    ...
    const handleDeleteConfirm = async () => {
        if (imageToDelete) {
            try {
                const response = await axios.delete(`${APP_URL}/api/delete-image/${imageToDelete.id}`, config);
                if (response.data.status === 200) {
    ...
    const handleSetPrimaryConfirm = async () => {
        if (imageToSetPrimary) {
            try {
                const response = await axios.put(`${APP_URL}/api/setprimaryimage/${imageToSetPrimary.id}`, {}, config);
                if (response.data.status === 200) {
                    setImages(images.map(img => ({
                        ...img,
                        is_primary_shop_image: img.id === imageToSetPrimary.id ? '1' : '0'
                    })));
        ...
        <div className="px-4 py-3 page-body">
            ...
            <div className='card'>
                <div className="card-header py-3 bg-transparent border-bottom-0 d-flex justify-content-between align-items-center">
                    <h6 className="title-font mt-2 mb-0" style={{ fontSize: '1.25rem' }}><strong>Welcome to {shopName}'s Gallery</strong></h6>
                    ...
                    <ul className="grid-wrapper li_animate mx-3">
                        {images.map((image, index) => (
                            <li
                                ...
                                style={{
                                    border: image.is_primary_shop_image === '1' ? '4px solid limegreen' : '0.5px solid #ddd', borderRadius: '10px'
                                }}
                            >
                                <img src={`${IMG_URL}/gallery/thumb/${image.image}`} alt={`Image ${index + 1}`} onClick={() => handleImageClick(image)} />
                                <button
                                ...
                                <button
                                    className="btn btn-sm position-absolute top-0 text-white m-2"
                                    style={{
                                        opacity: hoveredImage === index ? 1 : 0,
                                        transition: 'opacity 0.3s ease-in-out',
                                        border: '1px solid #fff',
                                        right: '14%',
                                        backgroundColor: image.is_primary_shop_image === '1' ? 'limegreen' : 'blue'
                                    }}
                                    onClick={(e) => handleSetPrimaryClick(image, e)}
                                    disabled={image.is_primary_shop_image === '1'}
                                >
            ...
            {showUploadForm && (
                <>
                    <div className="modal-backdrop show"></div>
                    <ImageUploadForm onAddImage={handleAddImage} onClose={handleModalClose} shopId={shopId} />
                </>
            )}
