...
const EditCatalogue = () => {
    ...
    // Catalogue and Shop details from query string
    ...
    useEffect(() => {
        const fetchCatalogueDetails = async () => {
            try {
                const response = await axios.get(`${APP_URL}/api/detailcatalog/${catalogId}`, {
                    headers: {
                ...
                if (response.data && response.data.Catalog) {
                    const catalogueData = response.data.Catalog;

                    // Set form values
                    setValue('item_title', catalogueData.item_title || '');
                    setValue('item_description', catalogueData.item_description || '');
                    setValue('platform_fee_type', catalogueData.platform_fee_type || '');
                    setValue('platform_fee', catalogueData.platform_fee || '');
                    setValue('price', catalogueData.price || '');
                    setValue('sale_price', catalogueData.sale_price || '');
                    setValue('focus_keyphrase', catalogueData.focus_keyphrase || '');
                    setValue('meta_description', catalogueData.meta_description || '');
                    setValue('seo_title', catalogueData.seo_title || '');
                    setValue('canonical_url', catalogueData.canonical_url || '');
                    setValue('seo_schema', catalogueData.seo_schema || '');
                    setValue('social_title', catalogueData.social_title || '');
                    setValue('social_description', catalogueData.social_description || '');
                } else {
                ...
                if (error.response?.data?.message && typeof error.response.data.message === 'object') {
                    ...
                    toast.error(`Error fetching catalogue data: ${errorMessages}`);
                } else {
                    // If it's a string or any other case, use the original approach
                    toast.error(`Error fetching catalogue data: ${error.response?.data?.message || error.message}`);
                }
            }
        };

        fetchCatalogueDetails();
    }, [APP_URL, catalogId, token, reset, setValue]);
    ...
    const onSubmit = async (data) => {
        ...
        try {
            ...
            const res = await axios.post(`${APP_URL}/api/editcatalog/${catalogId}`, formData, {
                headers: {
                    Authorization: `Bearer ${token}`,
        ...
        <div className="px-4 py-3 page-body">
            ...
            <div className='card'>
                <div className="card-header py-3 bg-transparent border-bottom-0">
                    <h6 className="title-font mt-2 mb-0" style={{ fontSize: '1.25rem' }}><strong>Edit Catalogue {catalogName}</strong></h6>
