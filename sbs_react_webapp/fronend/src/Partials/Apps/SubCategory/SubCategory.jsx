...
const SubCategory = () => {
    ...
    const [subCategoryToUpdatePopular, setSubCategoryToUpdatePopular] = useState(null);
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await axios.get(`${APP_URL}/api/subcategory`, {
                    headers: {
                        Authorization: `Bearer ${token}`,
                        "Content-Type": "application/json"
                    }
                });
                if (response.status === 200 && response.data.subcategory) {
    ...
    const handleEdit = (subCategoryName, subCategoryId) => {
        navigate(`/admin/editSubCategory/${subCategoryName}/${subCategoryId}`)
    }

    // Handle delete callback
    const handleDelete = useCallback((subCategoryName, id) => {
        setSubCategoryToDelete({ subCategoryName, id });
    ...
    const handleConfirmDelete = async () => {
        if (subCategoryToDelete) {
            setIsDeleting(true);
            try {
                const response = await axios.delete(`${APP_URL}/api/delete-subcategory/${subCategoryToDelete.id}`, {
                    headers: {
                        Authorization: `Bearer ${token}`,
                        "Content-Type": "application/json;"
                    }
                });
                setData(prevData => prevData.filter(subcategory => subcategory.id !== subCategoryToDelete.id));
    ...
    const handleConfirm = useCallback(async (id) => {
        try {
            const response = await axios.put(
                `${APP_URL}/api/update-subcategory-status/${id}`,
                null,
                {
                    headers: {
                        Authorization: `Bearer ${token}`,
            ...
            if (response.data.status === 200) {
                setData((prevData) =>
                    prevData.map((subcategory) =>
                        subcategory.id === id ? { ...subcategory, status: response.data.new_status.toString() } : subcategory
                    )
                );
                toast.success(response.data.message);
            } else {
            ...
            toast.error(error.response?.data?.message || "An error occurred while updating Sub category status");
        }
    }, [APP_URL, token]);

    const handlePopularClick = useCallback((category) => {
        setSubCategoryToUpdatePopular(category);
        setIsPopularModalOpen(true);
    }, []);

    const handleConfirmPopular = async () => {
        if (!subCategoryToUpdatePopular) return;

        setIsUpdatingPopular(true);
        try {
            const newPopularStatus = subCategoryToUpdatePopular.is_nav_menu === '1' ? '0' : '1';
            const response = await axios.put(`${APP_URL}/api/update-subcategory-isnavmenu/${subCategoryToUpdatePopular.id}`,
            ...
            if (response.status === 200) {
                setData(prevData => prevData.map(category =>
                    category.id === subCategoryToUpdatePopular.id ? { ...category, is_nav_menu: newPopularStatus } : category
                ));
            ...
            setSubCategoryToUpdatePopular(null);
        }
    };
    ...
    const columns = useMemo(
        () => [
            ...
            },
            {
                ...
                Cell: ({ row }) => (
                    <div className="d-flex align-items-center">
                        <img
                            src={row.original.image ? `${Img_url}/subcategory/list/${row.original.image}` : `${Img_url}/default/list/category.webp`}
                            alt={row.original.subcategory_name || "Sub Category Image"}
            ...
            },
            {
                ...
                Cell: ({ value, row }) => (
                    <button
                        ...
                        className={`btn btn-sm ${value === 1 || value === '1' ? 'btn-success' : 'btn-danger'}`}
                        style={{
                            backgroundColor: value === 1 || value === '1' ? '#28a745' : '#dc3545',
                            borderColor: value === 1 || value === '1' ? '#28a745' : '#dc3545',
                            color: '#ffffff',
                            width: '70px',
                            height: '35px'
                        }}
            ...
            },
            {
                ...
                Cell: ({ row }) => (
                    <div>
                        <button type="button" onClick={() => handleEdit(row.original.subcategory_name, row.original.id)} className="btn text-info px-2 me-1">
                            <i className="bi bi-pencil"></i>
                        </button>
                        <button type="button" onClick={() => handleDelete(row.original.subcategory_name, row.original.id)} className="btn text-danger px-2">
        ...
        <div className="px-4 py-3 page-body">
            ...
            <div className="col-lg-12 col-md-12">
                <div className="card mb-3 p-3">
                    <h4 className="title-font mb-3"><strong>Sub Category</strong></h4>

                    <div className="mb-3">
                        <Link className="btn btn-primary" to="/admin/addSubCategory">Add Sub Category</Link>
                    </div>
                    ...
                    <Modal
                        ...
                        message={`Are you sure you want to ${recordToUpdate?.status === '1' ? 'deactivate' : 'activate'} this category?`}
                    />
                    ...
                    {subCategoryToDelete && (
                        <DeleteModal
                            ...
                            message={`Are you sure you want to delete sub category ${subCategoryToDelete.subCategoryName}?`}
                            isLoading={isDeleting}
                        />
                    )}

                    {subCategoryToUpdatePopular && (
                        <PopularModal
                            isOpen={isPopularModalOpen}
                            onClose={() => setIsPopularModalOpen(false)}
                            onConfirm={handleConfirmPopular}
                            message={`Are you sure you want to mark this sub category item as ${subCategoryToUpdatePopular.is_nav_menu === '1' ? 'regular' : 'popular'}?`}
