...
const EditCatalogue = () => {
    ...
    useEffect(() => {
        const fetchCatalogueDetails = async () => {
            try {
                const response = await axios.get(`${APP_URL}/ma-vendor-detail-catalog/${catalogueId}`, {
                    headers: {
    ...
    const onSubmit = async (data) => {
        ...
        try {
            ...
            const res = await axios.post(`${APP_URL}/ma-edit-catalog/${catalogueId}`, formData, {
                headers: {
                    Authorization: `Bearer ${token}`,
            ...
            if (typeof errorMessage === 'object') {
                ...
                toast.error(`Error updating catalogue: ${errorMessages}`);
            } else {
                toast.error(`Error updating catalogue: ${errorMessage || error.message}`);
            }
        } finally {
    ...
    return (
        <div className="max-w-4xl mx-auto p-6 bg-white text-black rounded-lg shadow-md">
            ...
            <div className="flex justify-between items-center mb-6">
                <h6 className="text-xl font-bold">Edit Catalogue {catalogueName}</h6>
                ...
                <form onSubmit={handleSubmit(onSubmit)}>
                    ...
                    <div className="mt-6">
                        <button
                            ...
                            type="submit"
                        >
                            {isSubmitting ? 'Updating...' : 'Update Catalogue'}
                        </button>
                    </div>
                </form>
            </div>
