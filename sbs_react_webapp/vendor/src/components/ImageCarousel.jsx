...
const ImageCarousel = ({ catalogueId }) => {
    ...
    const fetchImages = async () => {
        try {
            ...
            console.error(`Error fetching images: ${err.response?.data?.message || err.message}`);
            setError(true);
        }
    };
    ...
    if (error || images.length === 0) {
        return (
            <div className="text-center text-gray-500 mb-8 mt-8">
                No gallery images found for this catalogue.
            </div>
        );
    }
    ...
    if (images.length === 1) {
        return (
            <div className="mb-8">
                <img
                    src={`${IMG_URL}/catalog-image/thumb/${images[0].image}`}
                ...
                <div className="grid gap-4"
                    ...
                    {images.map((image) => (
                        <div key={image.id} className="relative h-80">
                            <img
                                src={`${IMG_URL}/catalog-image/thumb/${image.image}`}
