...
const ShopGallery = () => {
    ...
    const handleDeleteConfirm = async () => {
        if (imageToDelete) {
            try {
                const response = await axios.delete(`${APP_URL}/ma-delete-gallery-image/${imageToDelete.id}`, {
                    headers: {
                        Authorization: `Bearer ${token}`,
                        "Content-Type": "application/json"
                    }
    ...
    const handleSetPrimaryConfirm = async () => {
        if (imageToSetPrimary) {
            try {
                const response = await axios.put(`${APP_URL}/ma-set-shop-primary-image/${imageToSetPrimary.id}`, {}, {
                    headers: {
                        Authorization: `Bearer ${token}`,
                        "Content-Type": "application/json"
                    }
        ...
        <div className="px-4 py-3">
            ...
            <div className='bg-white text-black rounded-lg shadow-md'>
                <div className="p-4 flex flex-col md:flex-row justify-between items-center">
                    <h6 className="text-2xl font-bold mb-4 md:mb-0">{`Welcome to ${slugToShopName(shopName)}'s Gallery`}</h6>
                    ...
                    <ul className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 p-4">
                        {images.map((image, index) => (
                            <li
                                ...
                                style={{
                                    border: image.is_primary_shop_image === '1' ? '4px solid limegreen' : 'none',
                                    borderRadius: '10px'
                                }}
                            >
                                <img
                                ...
                                <button
                                    className={`absolute top-2 right-14 ${image.is_primary_shop_image === '1' ? 'bg-green-500' : 'bg-blue-500'
